...
def _builtin_not_supported(*args):
    """
    Provides a more accurate error message when unsupported builtin methods are used.
    e.g. If Python has a C implementation which is calling a Python function such as `__import__`.
    """
    raise InvalidSmartContractError("Unsupported builtin used")


# We use a cache here as the set of allowed builtins is statically defined for each contract
# language version. There is no need to iterate through all builtins in python for each hook
# execution. This cache will contain a single entry for each Contracts API version, the maxsize of 3
# is decided as currently there are only 2 unique sets between all Contracts API versions.
@lru_cache(maxsize=3)
def _get_builtins(
    allowed_builtins: dict,
) -> dict:
    """
    Maps builtin name to builtin if the builtin is allowed.
    If the builtin is not allowed then the builtin is mapped to the _builtin_not_supported function.
    """
    return {
        k: v if k in allowed_builtins else _builtin_not_supported
...
def make_contract_version_sandbox(
    ...
    types_dict = TypeRegistry(
        builtins=_get_builtins(contract_lib.ALLOWED_BUILTINS),

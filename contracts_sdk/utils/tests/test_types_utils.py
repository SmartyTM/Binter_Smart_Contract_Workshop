...
class TestStrictInterface(unittest.TestCase):
    def test_verify_with_all_abstract_methods(self):
        class BaseClass(ABC):
            @abstractmethod
        ...
        types_utils.verify_public_methods_have_abstract_methods(DerivedClass)

    def test_verify_with_all_abstract_methods_multiple_inheritance(self):
        class BaseClass(ABC):
            @abstractmethod
            def base_method_one(self):
                pass
    ...
    def test_error_if_method_not_in_base_class(self):
        class BaseClass(ABC):
            @abstractmethod
            def base_method_one(self):
                pass
...
class PublicTypeValidationHelpersTest(unittest.TestCase):
    ...
    def test_validate_type_enum_type_valid(self):
        item = BooleanEnum.TRUE
        self.assertIsNone(types_utils.validate_type(item, BooleanEnum))

    def test_validate_type_enum_type_as_int_invalid(self):
        item = 1
        with self.assertRaises(exceptions.StrongTypingError) as ex:
            types_utils.validate_type(item, BooleanEnum)
        self.assertEqual("Expected BooleanEnum, got '1' of type int", str(ex.exception))

    def test_validate_type_enum_type_as_string_invalid(self):
        item = "false"
        with self.assertRaises(exceptions.StrongTypingError) as ex:
            types_utils.validate_type(item, BooleanEnum)
        self.assertEqual("Expected BooleanEnum, got 'false' of type str", str(ex.exception))

    def test_validate_type_enum_type_as_bool_invalid(self):
        item = True
        with self.assertRaises(exceptions.StrongTypingError) as ex:
            types_utils.validate_type(item, BooleanEnum)
        self.assertEqual("Expected BooleanEnum, got 'True' of type bool", str(ex.exception))


class ContractsLanguageDunderMixinTests(unittest.TestCase):
    class EqualityObject(types_utils.ContractsLanguageDunderMixin):
        def __init__(self, a, b, c):
            self.a = a
            # EqualityMixin doesn't differentiate between public and private attributes
            # - be careful if you have _cache attributes in your class
            self._b = b
            self.__c = c

    def test_equality(self):
        obj = self.EqualityObject(1, 2, 3)
        other_obj = self.EqualityObject(1, 2, 3)
        self.assertEquals(obj, other_obj)

    def test_unequal_a(self):
        obj = self.EqualityObject(1, 2, 3)
        other_obj = self.EqualityObject(42, 2, 3)
        self.assertNotEquals(obj, other_obj)

    def test_unequal_b(self):
        obj = self.EqualityObject(1, 2, 3)
        other_obj = self.EqualityObject(1, 42, 3)
        self.assertNotEquals(obj, other_obj)

    def test_unequal_c(self):
        obj = self.EqualityObject(1, 2, 3)
        other_obj = self.EqualityObject(1, 2, 42)
        self.assertNotEquals(obj, other_obj)

    def test_it_can_repr_a_class_with_public_attributes(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            ...
            def _public_attributes(cls):
                return [
                    types_utils.ValueSpec(name="pA", type="str", docstring=""),
                    types_utils.ValueSpec(name="pB", type="str", docstring=""),
                ]

        obj = TestClass("a_value", "b_value")
        expected = "TestClass(pA='a_value', pB='b_value')"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_repr_for_public_attributes_of_not_type_List_of_ValueSpec(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB
        ...
        obj = TestClass("a_value", "b_value")
        expected = "TestClass(!EXCEPTION_ON_SERIALIZING_ATTRIBUTES!)"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_it_can_repr_a_class_with_only_class_spec(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB
    ...
    def test_it_can_repr_a_class_with_only_class_spec_without_public_attributes(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB
    ...
    def test_repr_for_a_class_spec_thats_not_of_type_ClassSpec(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB
    ...
    def test_repr_for_defaultdict_instance_attribute(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            ...
            def _public_attributes(cls):
                return [
                    types_utils.ValueSpec(name="pA", type="defaultdict[str, bool]", docstring=""),
                ]

        obj = TestClass(defaultdict(lambda: False))
        expected = "TestClass(pA={})"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_it_does_not_print_attributes_not_in_public_attributes(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB
            ...
            def _public_attributes(cls):
                return [
                    types_utils.ValueSpec(name="pA", type="str", docstring=""),
                    types_utils.ValueSpec(name="pB", type="str", docstring=""),
                    # pC is not part of the public attributes list
                ]
    ...
    def test_it_can_print_class_level_attributes(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            p_class_level = "class level value"
            ...
            def _public_attributes(cls):
                return [
                    types_utils.ValueSpec(name="pA", type="str", docstring=""),
                    types_utils.ValueSpec(name="pB", type="str", docstring=""),
                    types_utils.ValueSpec(name="p_class_level", type="str", docstring=""),
                ]

        obj = TestClass("a_value", "b_value")
        expected = "TestClass(pA='a_value', pB='b_value', p_class_level='class level value')"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_it_can_print_properties(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA) -> None:
                self._pA = pA
        ...
        obj = TestClass("a_value")
        expected = "TestClass(pA='a_value')"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_it_generates_output_without_failing_when_exception_occurs(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self._pA = pA
                self.pB = pB
            ...
            @property
            def pA(self):
                raise ("This exception is intended to fail repr output, when property is accessed")
        ...
        obj = TestClass("a_value", "b_value")
        expected = "TestClass(pA='!EXCEPTION_ON_pA!', pB='b_value')"
        actual = repr(obj)
        self.assertEqual(expected, actual)

    def test_it_generates_basic_output_when_class_dont_have_public_attributes_method(self):
        class TestClass(types_utils.ContractsLanguageDunderMixin):
            def __init__(self, pA, pB) -> None:
                self.pA = pA
                self.pB = pB

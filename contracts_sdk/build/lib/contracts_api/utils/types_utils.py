...
def verify_public_methods_have_abstract_methods(
    cls: Type, allowed_non_abstract_methods: list[str] = []
):
    """
    `verify_public_methods_have_abstract_methods` ensures that all public
    functions (not starting with _) have a class in their
    inheritence tree with an associated abstract method for that function.
    """
    ...
    for name, _ in inspect.getmembers(cls, predicate=inspect.isfunction):
        ...
        if name in abstract_methods:
            continue
        if name in allowed_non_abstract_methods:
            continue
...
def get_iterator(
    items: Iterable[T], hint: str, name, check_empty=False, iterable_hint: Optional[str] = "list"
) -> Iterator[T]:
    exception_message = f"Expected {iterable_hint} of {hint} objects for '{name}', got '{items}'"
    ...
    if check_empty and not items:
        raise exceptions.InvalidSmartContractError(
            f"'{name}' must be a non empty {iterable_hint}, got {items}"
        )
    try:
        iterator = iter(items)
    except TypeError:
...
def validate_type(
    ...
    *,
    ...
    if check_empty and expected == str and item.strip() == "":
        message = (
...
class ContractsLanguageDunderMixin:
    """
    Mixin class to help with __repr__ and __eq__ implementation.
    * __repr__ implementations work when the object has
    a '_public_attributes(...)' method or
    a '_spec(...)' method defined

    * __eq__ implementation covers all simple cases.
    Note that all private attributes also count,
    so you need to be careful with cached values.

    How to use:
    class MyClass(ContractsLanguageDunderMixin):
        def _public_attributes(...):
        ....
        # no need to define __repr__ here as it comes from the mixin class
        # no need to define __eq__ here as it comes from the mixin class
    """
    ...
    def __repr__(self) -> str:
        def _get_attr_safely(name):
            try:
                attr = getattr(self, name)
                if isinstance(attr, defaultdict):
                    # unpack dict object to avoid __repr__ returning the
                    # default_factory function in repr output
                    attr = dict(attr)
                return attr
            except Exception:
                return "!EXCEPTION_ON_{}!".format(name)

        def _get_public_attributes(cls) -> List[ValueSpec]:
            try:
                return cls._public_attributes()
            except (AttributeError, TypeError):
                pass

            try:
                return cls._spec().public_attributes.values()
            except (AttributeError, TypeError):
                pass

            return []

        def _get_args_sting(public_attributes: List[ValueSpec]) -> str:
            try:
                values_to_print = {
                    value_spec.name: _get_attr_safely(value_spec.name)
                    for value_spec in public_attributes
                }
                return ", ".join([f"{key}={value!r}" for key, value in values_to_print.items()])
            except (AttributeError, TypeError):
                return "!EXCEPTION_ON_SERIALIZING_ATTRIBUTES!"

        public_attributes = _get_public_attributes(self.__class__)
        args_string = _get_args_sting(public_attributes)
